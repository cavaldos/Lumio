//! C-ABI wrapper over `lumio_matching::fuzzy_score` so macOS ranks identically to
...
/// `lumio_matching::fuzzy_score`, adapted to the C ABI. Both arguments must be
/// pre-lowercased by the caller (`fuzzy_score` is case-sensitive).
pub(crate) fn lumio_fuzzy_score_impl(query: *const c_char, title: *const c_char) -> i64 {
    let query = cstr_to_string(query);
    let title = cstr_to_string(title);
    lumio_matching::fuzzy_score(&query, &title).unwrap_or(NO_MATCH)
}
...
mod tests {
    ...
    fn score(query: &str, title: &str) -> i64 {
        ...
        lumio_fuzzy_score_impl(q.as_ptr(), t.as_ptr())
    }
    ...
    fn null_pointers_do_not_crash() {
        assert_eq!(
            lumio_fuzzy_score_impl(std::ptr::null(), std::ptr::null()),
            2_000,
