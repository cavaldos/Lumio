...
enum LauncherBlurMaterial: String, CaseIterable, Codable, Identifiable {
    ...
    var detail: String {
        ...
        case .menu: return "Neutral default lumio"
...
/// `~/.lumio/config` under `running_apps_placement`. The setting is now a simple
...
struct ThemeSettings: Codable, Equatable {
    ...
    /// `0` squares every corner. Persisted in `~/.lumio/config` under
    /// `ui_surface_radius`.
