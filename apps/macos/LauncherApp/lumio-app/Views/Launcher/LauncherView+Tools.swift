...
/// their config. Composition lives in core (`lumio-tools`); this decides which
...
extension LauncherView {
    ...
    /// launched app does ask, but Lumio is an accessory app that stays *active*
    /// after ordering its window out, and one process's request does not
    /// preempt the active app. Lumio is the only party here holding activation,
    /// so Lumio is the one that can pass it on.
    ///
    /// Ordered deliberately: this runs while Lumio is still active, because a
