            ...
            HStack(alignment: .top, spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(definition.query)
                        .font(.system(size: CGFloat(themeStore.settings.fontSize + 6), weight: Font.Weight.semibold, design: .serif))
                        .foregroundStyle(themeStore.fontColor())
                    Text(definition.sourceLabel.uppercased())
                        .font(.system(size: CGFloat(themeStore.settings.fontSize - 2), weight: Font.Weight.bold, design: .rounded))
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                Text(section.label.uppercased())
                    .font(.system(size: CGFloat(themeStore.settings.fontSize - 1), weight: Font.Weight.bold, design: .serif))
                    .foregroundStyle(themeStore.secondaryTextColor())
            ...
            if let translated = section.translated, !translated.isEmpty {
                Text(translated)
                    .font(.system(size: CGFloat(themeStore.settings.fontSize + 2), weight: Font.Weight.regular, design: .serif))
                    .foregroundStyle(themeStore.secondaryTextColor())
                    .lineSpacing(CGFloat(themeStore.settings.fontSize * 0.15))
                    .textSelection(.enabled)
            } else if section.failed {
                Text("No translation")
                    .font(.system(size: CGFloat(themeStore.settings.fontSize), weight: Font.Weight.regular, design: .serif))
                    .foregroundStyle(themeStore.mutedTextColor())
            }
