...
nonisolated private let openLaunchLog = Logger(subsystem: "noah-code.Lumio", category: "open")
        ...
        if pickedKeys.isEmpty,
           ...
           DeleteTargetLogic.isTrashPath(selected.path, homeDirectory: NSHomeDirectory()) {
            ...
            guard let count = EmptyTrashCommand.itemCount() else {
                showBanner(
                    "Allow Lumio to control Finder in System Settings ▸ Privacy ▸ Automation to empty the Trash",
                    style: .error,
                    duration: 2.8
                )
                return
