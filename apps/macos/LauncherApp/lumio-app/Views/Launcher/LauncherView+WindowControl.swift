...
private let hotkeyLog = Logger(subsystem: "noah-code.Lumio", category: "hotkey")
...
extension LauncherView {
    ...
    func activateLauncherModeAndFocus() {
        ...
        // Lumio should not silently drop the user back to home. Just restore focus
    ...
    /// Show for a `lumio <mode>` delivery, from hidden or from the background.
    ///
    /// Never activates before capturing: `captureFrontmostAppForRestoreIfNeeded`
    /// nils the pid when Lumio is already frontmost, and then Esc has nothing to
