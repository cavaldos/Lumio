...
/// Empties the macOS Trash via Finder. `~/.Trash` is TCC-protected, so Lumio
...
struct EmptyTrashCommand {
    /// Whether Lumio already has permission to automate Finder - checked WITHOUT
    ...
    nonisolated private static func runFinder(_ body: String) -> (NSAppleEventDescriptor?, String?) {
        ...
        if let errorInfo {
            ...
            if code == -1743 {
                return (nil, "Allow Lumio to control Finder in System Settings ▸ Privacy ▸ Automation")
            }
