...
@_silgen_name("lumio_modes_list_text")
nonisolated
private func lumio_modes_list_text() -> UnsafeMutablePointer<CChar>?

@_silgen_name("lumio_modes_parse_json")
nonisolated
private func lumio_modes_parse_json(_ argvJSON: UnsafePointer<CChar>?) -> UnsafeMutablePointer<CChar>?

@_silgen_name("lumio_free_cstring")
nonisolated
private func lumio_free_cstring(_ ptr: UnsafeMutablePointer<CChar>?)

/// `lumio clipboard`: open the launcher already in a mode. The table and the
/// argv grammar live in core (`core/engine/modes.rs`); this carries the answer
/// across and nothing else.
enum LaunchModes {
    /// Scoped to this bundle id so `lumio` drives the installed app and
    /// `lumiodev` drives Lumio Dev, rather than whichever answers first.
    static var deliveryNotification: Notification.Name {
        Notification.Name("lumio.launchQueryDelivered.\(bundleID)")
    }
    ...
    static func handleLaunchArguments() -> Int32? {
        ...
        case .unknownMode(let name):
            FileHandle.standardError.write(
                Data("lumio: unknown mode \"\(name)\"\n\n\(listText())".utf8))
            return 2

        case .unavailableMode(let name):
            FileHandle.standardError.write(
                Data("lumio: mode \"\(name)\" is not available on this platform\n".utf8))
            return 2
    ...
    private static func parse(_ arguments: [String]) -> Launch {
        guard
            ...
            let decision = call(lumio_modes_parse_json, with: String(decoding: argv, as: UTF8.self)),
    ...
    private static func listText() -> String {
        guard let ptr = lumio_modes_list_text() else { return "" }
