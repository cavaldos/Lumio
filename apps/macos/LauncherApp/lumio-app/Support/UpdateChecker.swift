...
/// Checks GitHub Releases for a newer version of Lumio and, if one exists,
/// publishes it so the UI can show a non-intrusive "update available" notice.
///
/// This is notify-only: Lumio is distributed via GitHub Releases, so we never
...
final class UpdateChecker: ObservableObject {
    ...
    private let latestReleaseURL = URL(string: "https://api.github.com/repos/cavaldos/Lumio/releases/latest")!
    private let session: URLSession
    private let defaults: UserDefaults
    private let log = Logger(subsystem: "noah-code.Lumio", category: "UpdateChecker")
    ...
    private static let lastCheckKey = "lumio.update.lastCheckEpoch"
    private static let dismissedVersionKey = "lumio.update.dismissedVersion"
    ...
    private func performCheck(currentVersion: String, isManual: Bool) async {
        ...
        do {
            ...
            let urlString = (json["html_url"] as? String) ?? "https://github.com/cavaldos/Lumio/releases/latest"
            ...
            if isManual { statusMessage = "Update available: Lumio \(latest)" }
    ...
    static let productionBundleID = "noah-code.Lumio"
