...
/// Info.plist. The CLI `-v` path in lumio_appApp has its own resilient reader for
...
enum AppVersion {
    ...
    /// bundle id to `noah-code.Lumio.Dev`). Dev builds carry a fixed 1.0 version,
    /// so comparing them against published releases is meaningless.
    static var isDevBuild: Bool {
        Bundle.main.bundleIdentifier?.hasSuffix(".Dev") ?? false
    }

    /// Human-readable label for display, e.g. "Lumio 1.0.0 (42)" or "Lumio 1.0.0".
    /// Dev builds are tagged so the version isn't mistaken for a real release.
    static var displayString: String {
        let suffix = isDevBuild ? " - dev" : ""
        guard let short else { return "Lumio (unknown version)\(suffix)" }
        if let build, build != short {
            return "Lumio \(short) (\(build))\(suffix)"
        }
        return "Lumio \(short)\(suffix)"
    }
}
