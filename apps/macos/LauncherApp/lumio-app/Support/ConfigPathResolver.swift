...
@_silgen_name("lumio_config_path")
nonisolated
private func lumio_config_path(_ dev: Bool) -> UnsafeMutablePointer<CChar>?

@_silgen_name("lumio_free_cstring")
nonisolated
private func lumio_free_config_cstring(_ ptr: UnsafeMutablePointer<CChar>?)

enum ConfigPathResolver {
    private static let productionBundleIdentifier = "noah-code.Lumio"
    ...
    /// `~/.lumio.config` into `~/.lumio/`. Two implementations of "where is the
    ...
    static func resolvedPath() -> String {
        ...
        if let custom = env["LUMIO_CONFIG_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
            !custom.isEmpty
        {
        ...
        guard let ptr = lumio_config_path(isDevBuild) else {
            ...
            return (home as NSString).appendingPathComponent(
                isDevBuild ? ".lumio/config.dev" : ".lumio.config")
        }
        defer { lumio_free_config_cstring(ptr) }
    ...
    private static var isDevBuild: Bool {
        ...
        return Bundle.main.bundleURL
            ...
            .path
            .lowercased()
            .contains("/lumio dev.app")
    }

    static func applyDefaultConfigEnvironmentIfNeeded() {
        let env = ProcessInfo.processInfo.environment
        if let existing = env["LUMIO_CONFIG_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines), !existing.isEmpty {
            return
        }

        let resolved = resolvedPath()
        setenv("LUMIO_CONFIG_PATH", resolved, 1)
    }
}
